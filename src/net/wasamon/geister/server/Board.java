...
public class Board {
    ...
    /**
     * Return encoded information of the item, xyc.
     * The item color is red/blue = R/B when secret, or red/blue/unknown = r/b/u when public. 
     * @param item target item to encode
     * @param mine a flag whether the item is mine or not 
     * @return encoded info. of the item
     */
    public String getEncodedItem(Item item, boolean mine){
	int x = item.getX();
	int y = item.getY();
	...
	for(int y = 0; y < Constant.SIZE; y++){
		...
		if(item != null){
		    ss = item.getName();
		    if(item.getPlayer() == getPlayer(opposite)){ // opposite one
			ss = ss.toLowerCase();
		    }
		}
		s += ss + " ";
