...
public class HumanGUIPlayer extends BasePlayer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if(args.length < 3){
            System.out.println("java -cp build/libs/geister.jar net.wasamon.geister.player.HumanGUIPlayer host 10000/10001 init");
            System.out.println("ex. java -cp build/libs/geister.jar net.wasamon.geister.player.HumanGUIPlayer localhost 10000 ABCD");
        }
        p.init(args[0], Integer.parseInt(args[1]));
        System.out.println(p.setRedItems(args[2]));
        p.printBoard();
        ...
        GAME_LOOP: while (true) {
            ...
            p.moveFlag = false;
            System.out.println(p.moveInst);
            System.out.println(p.move(p.moveInst));
            p.printBoard();
        }
...
class Canvas extends JPanel implements MouseListener, MouseMotionListener{
    ...
    private void paintItems(Graphics2D g){
        ...
        for(int i = 0; i < player.enemy.length; i++){
            P p = player.enemy[i];
            g.drawImage(img_u, convX(5-p.x), convY(5-p.y), dim, dim, null); // to rotate
        }
        at.rotate(180 * Math.PI / 180.0, DIM/2, DIM/2);
        g.setTransform(at);
        for(int i = 0; i < player.enemy.length; i++){
            if(selected != i){
                P p = player.own[i];
                g.drawImage(img_u, convX(p.x), convY(p.y), dim, dim, null);
            }
        }
        if(selected != -1){
            g.drawImage(img_u, selectedX, selectedY, dim, dim, null);
        }
    }
    ...
    private P conv(int x, int y){
        return new P((x-dim)/dim, (y-dim)/dim);
    }
    ...
    private boolean isMovable(P p0, P p1){
        ...
        if(p1.x < 0 || p1.x > 6 || p1.y < 0 || p1.y > 6) return false; // outside
        if(p0.x == p1.x && p0.y == p1.y) return false; // same place
        if(p0.x != p1.x && p0.y != p1.y) return false; // not same row and column
        if(Math.abs(p0.x - p1.x) > 1 || Math.abs(p0.y- p1.y) > 1) return false; // allowed only just neighbor
        if(isOwnOccupied(p1.x, p1.y)) return false; // own occupied
        return true;
    }
    
    private String moveInst(P p0, P p1){
        String label = new String(new char[]{(char)('A' + selected)});
        if(p0.x == p1.x && p0.y+1 == p1.y) return label + ",S";
        if(p0.x == p1.x && p0.y-1 == p1.y) return label + ",N";
        if(p0.x+1 == p1.x && p0.y == p1.y) return label + ",E";
        if(p0.x-1 == p1.x && p0.y == p1.y) return label + ",W";
        return "ERROR";
    }
